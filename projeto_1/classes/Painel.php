...
class Painel
{
	public static $cargos = ['0'=> 'Normal', '1' => 'Colaborador', '2'=> 'Administrador']; 
	...
	public static function listarUsuariosOnline(){
		...
		$sql= MySql::conectar()->prepare("SELECT * FROM `tb_adm_online`");
		$sql->execute();
		return $sql->fetchAll();
	}
	...
	public static function limparUsuariosOnline(){
		...
		$sql= Mysql::conectar()->exec("DELETE FROM `tb_adm_online`  WHERE ultima_acao < '$date' - INTERVAL 1 MINUTE");
	}
	...
	public static function imagemValidaIcon($imagem){
	...
	public static function uploadfileSlide($file){
	...
	public static function deleteFileSlide($file){
		@unlink('../imagens/'.$file);
	}
	...
	public static function uploadfileIcon($file){
		$formatoArquivo = explode('.',$file['name']);
		$imagemNome = uniqid().'.'.$formatoArquivo[count($formatoArquivo)-1];
		if(move_uploaded_file($file['tmp_name'],'../icons/'.$imagemNome))
			return	$imagemNome;
		else
			return false;
	}

	public static function deleteFileIcon($file){
		@unlink('../icons/'.$file);
	}
